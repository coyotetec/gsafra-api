...
export default {
  async index(request: Request, response: Response) {
    ...
    const planejamentosAtividadeInsumosByEmpresas = await Empresa.findAll({
      where: {
        id_cliente_empresa: empresa.id_cliente_empresa,
      },
      include: {
        association: 'planejamentos_atividade_insumos',
        where: {
          ...(data_atualizacao && {
            data_atualizacao: {
    ...
    const planejamentosAtividadeInsumos: any[] = [];

    planejamentosAtividadeInsumosByEmpresas.forEach(
      (planejamentosAtividadeInsumosByEmpresa: any) => {
        planejamentosAtividadeInsumos.push(
          ...planejamentosAtividadeInsumosByEmpresa.planejamentos_atividade_insumos,
        );
      },
    );

    return response.json(planejamentosAtividadeInsumos);
  },
};
